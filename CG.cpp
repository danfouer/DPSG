...
r<0:ep在矢量opsp的顺时针方向 
*******************************************************************************/ 
double multiply(CGPOINT sp,CGPOINT ep,CGPOINT op) 
{ 
...
*******************************************************************************/ 
double dotmultiply(CGPOINT p1,CGPOINT p2,CGPOINT p0) 
{ 
return ((p1.x-p0.x)*(p2.x-p0.x)+(p1.y-p0.y)*(p2.y-p0.y)); 
} 

/* 判断点p是否在线段l上，条件：(p在线段l所在的直线上)&& (点p在以线段l为对角线的矩形内) */ 
bool online(LINESEG l,CGPOINT p) 
{ 
return((multiply(l.e,p,l.s)==0) 
&&( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) ); 
} 
bool onlineregion(LINESEG l,CGPOINT p) //点p在以线段l为对角线的矩形内
{
	return(( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) ); 
}
// 返回点p以点o为圆心逆时针旋转alpha(单位：弧度)后所在的位置 
CGPOINT rotate(CGPOINT o,double alpha,CGPOINT p) 
{ 
CGPOINT tp; 
p.x-=o.x; 
p.y-=o.y; 
...
可以用于求线段之间的夹角 
*/ 
double angle(CGPOINT o,CGPOINT s,CGPOINT e) 
{ 
...
*/ 
double relation(CGPOINT p,LINESEG l) 
{ 
LINESEG tl; 
tl.s=l.s; 
tl.e=p; 
...
// 求点C到线段AB所在直线的垂足 P 
CGPOINT perpendicular(CGPOINT p,LINESEG l) 
{ 
...
double ptolinesegdist(CGPOINT p,LINESEG l,CGPOINT &np) 
{ 
double r=relation(p,l); 
if(r<0) 
{ 
...
double ptoldist(CGPOINT p,LINESEG l) 
{ 
...
double ptoCGPOINTset(int vcount,CGPOINT CGPOINTset[],CGPOINT p,CGPOINT &q) 
{ 
int i; 
...
bool CircleInsidePolygon(int vcount,CGPOINT center,double radius,CGPOINT polygon[]) 
{ 
CGPOINT q; 
double d; 
q.x=0; 
q.y=0; 
d=ptoCGPOINTset(vcount,polygon,center,q); 
...
LINE makeline(CGPOINT p1,CGPOINT p2) 
{ 
   LINE tl; 
   int sign = 1; 
   tl.a=p2.y-p1.y; 
...
// 求点p关于直线l的对称点  
CGPOINT symmetry(LINE l,CGPOINT p) 
{ 
   CGPOINT tp; 
...
bool lineintersect(LINE l1,LINE l2,CGPOINT &p) // 是 L1，L2 
{ 
...
bool intersection(LINESEG l1,LINESEG l2,CGPOINT &inter) 
{ 
LINE ll1,ll2; 
...
本程序默认第一个条件已经满足 
*/ 
bool issimple(int vcount,CGPOINT polygon[]) 
{ 
int i,cn; 
LINESEG l1,l2; 
for(i=0;i<vcount;i++) 
...
void checkconvex(int vcount,CGPOINT polygon[],bool bc[]) 
{ 
int i,index=0; 
CGPOINT tp=polygon[0]; 
...
bool isconvex(int vcount,CGPOINT polygon[]) 
{ 
bool bc[MAXV]; 
...
double area_of_polygon(int vcount,CGPOINT polygon[]) 
{ 
int i; 
double s; 
...
// 如果输入顶点按逆时针排列，返回true 
bool isconterclock(int vcount,CGPOINT polygon[]) 
{ 
return area_of_polygon(vcount,polygon)>0; 
} 
// 另一种判断多边形顶点排列方向的方法  
bool isccwize(int vcount,CGPOINT polygon[]) 
{ 
   int i,index; 
   CGPOINT a,b,v; 
   v=polygon[0]; 
...
*******************************************************************************/
bool PointInsidePolygon(int vcount,CGPOINT center,CGPOINT polygon[]) 
{
	int count=0;
	int i; 
	LINESEG l; 
	LINESEG Lcenter;
	Lcenter.e.x=center.x;
	Lcenter.e.y=center.y ;
	Lcenter.s.x= 0;
	Lcenter.s.y= center.y;
	CGPOINT LineInter;
	for(i=0;i<vcount-1;i++) 
	{ 
		l.s=polygon[ i ]; 
		...
		if(l.s.y!=l.e.y)
		{
			...
			else if(intersection(Lcenter,l,LineInter))
			{
				count=count+1;
			}
		}
...
/* 射线法判断点q与多边形polygon的位置关系，要求polygon为简单多边形，顶点逆时针排列 
   ...
   如果点在多边形外： 返回2 
*/ 
int insidepolygon(int vcount,CGPOINT Polygon[],CGPOINT q) 
{ 
int c=0,i,n; 
LINESEG l1,l2; 
...
bool InsideConvexPolygon(int vcount,CGPOINT polygon[],CGPOINT q) // 可用于三角形！ 
{ 
CGPOINT p; 
LINESEG l; 
int i; 
...
void Graham_scan(CGPOINT CGPOINTSet[],CGPOINT ch[],int n,int &len) 
{ 
int i,j,k=0,top=2; 
CGPOINT tmp; 
// 选取CGPOINTSet中y坐标最小的点CGPOINTSet[k]，如果这样的点有多个，则取最左边的一个 
for(i=1;i<n;i++) 
if ( CGPOINTSet[i].y<CGPOINTSet[k].y || 
(CGPOINTSet[i].y==CGPOINTSet[k].y) && (CGPOINTSet[i].x<CGPOINTSet[k].x) ) 
k=i; 
tmp=CGPOINTSet[0]; 
CGPOINTSet[0]=CGPOINTSet[k]; 
CGPOINTSet[k]=tmp; // 现在CGPOINTSet中y坐标最小的点在CGPOINTSet[0] 
for (i=1;i<n-1;i++) /* 对顶点按照相对CGPOINTSet[0]的极角从小到大进行排序，极角相同 
的按照距离CGPOINTSet[0]从近到远进行排序 */ 
{ 
k=i; 
for (j=i+1;j<n;j++) 
if ( multiply(CGPOINTSet[j],CGPOINTSet[k],CGPOINTSet[0])>0 ||  // 极角更小    
      (multiply(CGPOINTSet[j],CGPOINTSet[k],CGPOINTSet[0])==0) && /* 极角相等， 
距离更短 */         dist(CGPOINTSet[0],CGPOINTSet[j])<dist(CGPOINTSet[0],CGPOINTSet[k]) ) 
k=j; 
tmp=CGPOINTSet[i]; 
CGPOINTSet[i]=CGPOINTSet[k]; 
CGPOINTSet[k]=tmp; 
} 
ch[0]=CGPOINTSet[0]; 
ch[1]=CGPOINTSet[1]; 
...
for (i=3;i<n;i++) 
{ 
while (multiply(CGPOINTSet[i],ch[top],ch[top-1])>=0) top--; 
ch[++top]=CGPOINTSet[i]; 
} 
len=top+1; 
} 
...
void ConvexClosure(CGPOINT CGPOINTSet[],CGPOINT ch[],int n,int &len) 
{ 
...
for(i=1;i<n;i++) 
{ 
if(CGPOINTSet[i].y<tmp.y||CGPOINTSet[i].y == tmp.y&&CGPOINTSet[i].x<tmp.x) 
{ 
index=i; 
} 
use[i]=false; 
} 
tmp=CGPOINTSet[index]; 
first=index; 
use[index]=true; 
...
*/ 
bool LinesegInsidePolygon(int vcount,CGPOINT polygon[],LINESEG l) 
{ 
// 判断线端l的端点是否不都在多边形内 
if(!insidepolygon(vcount,polygon,l.s)||!insidepolygon(vcount,polygon,l.e)) 
return false; 
int top=0,i,j; 
CGPOINT CGPOINTSet[MAXV],tmp; 
LINESEG s; 
...
for(j=i+1;j<top;j++) 
{ 
if( CGPOINTSet[i].x>CGPOINTSet[j].x || fabs(CGPOINTSet[i].x-CGPOINTSet[j].x)<EP && CGPOINTSet[i].y>CGPOINTSet[j].y ) 
   { 
tmp=CGPOINTSet[i]; 
CGPOINTSet[i]=CGPOINTSet[j]; 
CGPOINTSet[j]=tmp; 
} 
} 
} 
...
for(i=0;i<top-1;i++) 
{ 
tmp.x=(CGPOINTSet[i].x+CGPOINTSet[i+1].x)/2; //得到两个相邻交点的中点 
tmp.y=(CGPOINTSet[i].y+CGPOINTSet[i+1].y)/2; 
...
CGPOINT cg_simple(int vcount,CGPOINT polygon[]) 
{ 
...
CGPOINT gravitycenter(int vcount,CGPOINT polygon[]) 
{ 
CGPOINT tp; 
...
结论： x坐标最大，最小的点肯定是凸顶点 
       y坐标最大，最小的点肯定是凸顶点            
*/ 
CGPOINT a_CGPOINT_insidepoly(int vcount,CGPOINT polygon[]) 
{ 
   CGPOINT v,a,b,r; 
   int i,index; 
   v=polygon[0]; 
...
*/ 
void CGPOINTtangentpoly(int vcount,CGPOINT polygon[],CGPOINT p,CGPOINT &rp,CGPOINT &lp) 
{ 
LINESEG ep,en; 
bool blp,bln; 
rp=polygon[0]; 
...
bool core_exist(int vcount,CGPOINT polygon[],CGPOINT &p) 
{ 
int i,j,k; 
LINESEG l; 
LINE lineset[MAXV]; 
...
*/ 
bool CGPOINT_in_circle(CGPOINT o,double r,CGPOINT p) 
{ 
double d2=(p.x-o.x)*(p.x-o.x)+(p.y-o.y)*(p.y-o.y); 
double r2=r*r; 
return d2<r2||abs(d2-r2)<EP; 
} 
double CGpolygon_on_circle(CGPOINT o,double r,int vcount,CGPOINT polygon[],int *PartCircle) 
{
	int i=0;
	int onCircle=0;
	double onCircleRate=0.0;
	double d2=0.0;
	double r2=0.0;
	*PartCircle=0;
	...
	for(i=0;i<vcount;i++)
	{
		d2=(polygon[i].x-o.x)*(polygon[i].x-o.x)+(polygon[i].y-o.y)*(polygon[i].y-o.y); 
		r2=r*r;
		if((abs(d2-r2))/r2<EPDPSG)
		{
			onCircle++;
			ContinueCounter++;
		}
		...
		{
			if(ContinueCounter>*PartCircle)
			{
				*PartCircle=ContinueCounter;
				ContinueCounter=0;
	...
	return((double)onCircle/vcount);
}
//double CGpolygon_on_circle(CGPOINT o,double r,int vcount,CGPOINT polygon[]) 
//{
//	int i=0;
//	int onCircle=0;
//	double onCircleRate=0.0;
...
*/ 
bool cocircle(CGPOINT p1,CGPOINT p2,CGPOINT p3,CGPOINT &q,double &r) 
{ 
...
bool line_circle(LINE l,CGPOINT o,double r,CGPOINT &p1,CGPOINT &p2) 
{ 
return true; 
} 
/************************** 
...
CGPOINT rect4th(CGPOINT a,CGPOINT b,CGPOINT c) 
{ 
CGPOINT d; 
...
First take the convex hull of the CGPOINTs. Let the resulting convex 
...
内含： return 5； 
*/ 
int CircleRelation(CGPOINT p1, double r1, CGPOINT p2, double r2) 
{ 
...
// 调用ptoldist函数，在第4页 
bool CircleRecRelation(CGPOINT pc, double r, CGPOINT pr1, CGPOINT pr2, CGPOINT pr3, CGPOINT pr4) 
{ 
...
// 两个点是否在直线同侧，是则返回true 
bool SameSide(CGPOINT p1, CGPOINT p2, LINE line) 
{ 
...
// 两圆已经相交（相切） 
void  c2CGPOINT(CGPOINT p1,double r1,CGPOINT p2,double r2,CGPOINT &rp1,CGPOINT &rp2) 
{ 
double a,b,r; 
a=p2.x-p1.x; 
b=p2.y-p1.y; 
...
// 必须保证相交 
double c2area(CGPOINT p1,double r1,CGPOINT p2,double r2) 
{ 
CGPOINT rp1,rp2; 
c2CGPOINT(p1,r1,p2,r2,rp1,rp2); 
double a,b,rr; 
...
//0----相离 1----相切 2----相交 
int clCGPOINT(CGPOINT p,double r,double a,double b,double c,CGPOINT &rp1,CGPOINT &rp2) 
{ 
int res=0; 
...
void incircle(CGPOINT p1,CGPOINT p2,CGPOINT p3,CGPOINT &rp,double &r) 
{ 
...
void cutCGPOINT(CGPOINT p,double r,CGPOINT sp,CGPOINT &rp1,CGPOINT &rp2) 
{ 
CGPOINT p2; 
p2.x=(p.x+sp.x)/2; 
p2.y=(p.y+sp.y)/2; 
